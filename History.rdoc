=== Version 0.3.0
* Enhancements
  * configuration is now accessible through Lolita.setup block
  * autocomplete updated to remove links and accept file location through configuration
=== Version 0.2.0
* Changes
  * added system_name field to Menu model
  * added autocomplete to MenuItem url field
=== Version 0.1.6
* Changes
  * new style generators and improved navigation
  
=== Version 0.0.2; 0.1.3 / 2011-09-06
* Bug fixes
  * Asset pipline improved and fixed
  
=== Version 0.1.1 / 2011-09-06
* Changes
  * Works with rails 3.1 pipeline
  
=== Version 0.1.0 / 2011-09-05
* Changes
  * Works with lolita 3.2.0 and rails 3.1.0
  
=== Version 0.0.15 / 2011-08-24
* Bug fixes
  * require problem in generate_urls fixed
  
=== Version 0.0.14 / 2011-08-24
* Bug fixes
  * reading form files fixed for autcomplete

=== Version 0.0.13 / 2011-08-24
* Bug fixes
  * Fixed rake task environment loading problem.
  
=== Version 0.0.12 / 2011-08-23
* Enhancements
  * Autocomplete for url added
  
=== Version 0.0.9 / 2011-06-10
* Enhancements
  * Active item detection based on request
  * Item urls with arguments

* Bug fixes
  * Delete works on newely added items
  
=== Version 0.0.8 / 2011-06-09
* Enhancements
  * Delete functionality
  
=== Version 0.0.7 / 2011-05-20
* Enhancements
  * Notification after operations with menu items.
  * Builder accept conditions
  
=== Version 0.0.6 / 2011-05-19
* Bug fixes
  * Can't recognize HAML files in projects without haml.
  
=== Version 0.0.5 / 2011-05-19
* Bug fixes
  * HAML dependencies not supported
  
=== Version 0.0.4 / 2011-05-19
* Bug fixes
  * Wrong table names for models

=== Version 0.0.3 /  2011-05-19
* Enhancements
  * README updated

=== Version 0.0.2 / 2011-05-19
* Bug fixes
  * migrations have one timestamp 
  * migration names does not match class names

=== Version 0.0.1 / 2011-05-19
  * Basic models, controllers, views
  * JavaScripts, Style, Migrations